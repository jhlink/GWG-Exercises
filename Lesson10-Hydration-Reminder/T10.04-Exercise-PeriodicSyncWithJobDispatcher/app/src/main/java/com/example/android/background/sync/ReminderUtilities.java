...
public class ReminderUtilities {
    ...
    // COMP (16) Create a synchronized, public static method called scheduleChargingReminder that
    // takes in a context. This method will use FirebaseJobDispatcher to schedule a job that
    // repeats roughly every REMINDER_INTERVAL_SECONDS when the phone is charging. It will
    // trigger WaterReminderFirebaseJobService
    // Checkout https://github.com/firebase/firebase-jobdispatcher-android for an example
    synchronized public static void scheduleChargingReminder(@NonNull final Context context) {

        // COMP (17) If the job has already been initialized, return
        if (sInitialized) {
            return;
        }

        // COMP (18) Create a new GooglePlayDriver
        Driver driver = new GooglePlayDriver(context);

        // COMP (19) Create a new FirebaseJobDispatcher with the driver
        FirebaseJobDispatcher dispatcher = new FirebaseJobDispatcher(driver);

        // COMP (20) Use FirebaseJobDispatcher's newJobBuilder method to build a job which:
        ...
        // COMP (21) Use dispatcher's schedule method to schedule the job
        dispatcher.schedule(constraintReminderJob);

        // COMP (22) Set sInitialized to true to mark that we're done setting up the job
        sInitialized = true;
    }
}
